...
class TestRenderVariables:
    ...
    def test_html_escape_escapes_sensitive_bytes(self) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(b"A&B", id="bytes"),
            pytest.param(memoryview(b"A&B"), id="memoryview"),
        ],
    )
    ...
    def test_lambda_returning_bytes_like_matches_direct_interpolation(
        self, value: bytes | memoryview
    ) -> None:
        compiled = fstache.compile(b"{{value}}|{{{value}}}|{{& value}}")
        direct = render_template(compiled, {"value": value}).to_bytes()
        via_lambda = render_template(
    ...
    def test_renders_unescaped_renderer_variable_to_bytes(self) -> None:
        content = render_template(
            fstache.compile(b"<{{name}}> {{suffix}}"),
            {"name": "A&B", "suffix": memoryview(b"done")},
        )
        compiled = fstache.compile(b"before {{{content}}}|{{& content}} after")

        assert render_template(compiled, {"content": content}).to_bytes() == (
            b"before <A&amp;B> done|<A&amp;B> done after"
        )

    def test_renders_escaped_renderer_variable_to_bytes(self) -> None:
        content = render_template(fstache.compile(b"<{{name}}>"), {"name": "A&B"})
    ...
    def test_resolve_missing_variable_exception_propagates(self) -> None:
        ...
        error = MissingVariableError("name")
    ...
    def test_resolve_missing_variable_output_follows_interpolation_escaping(
        ...
        assert render_template(
            ...
            b"&lt;strong&gt;fallback&lt;/strong&gt;"
            b"|<strong>fallback</strong>|<strong>fallback</strong>"
        )
